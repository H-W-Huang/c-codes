...
{
    int data;       //数据
    struct TNode *lchild;    //左儿子
    struct TNode *rchild;    //右儿子
}TNode, *Tree;
...
//交换左右子树
int Switch_children(Tree T)
{
    if(T==NULL) return 0;
    else 
    {
        Tree p;
        Switch_children(T->lchild);
        Switch_children(T->rchild);
        p=T->lchild;
        T->lchild=T->rchild;
        T->rchild=p;
        return 1;
...
int Leaves_of_Tree(Tree T)
{
    int num=0;
    if(T==NULL) return 0;
    else 
    {
        num=Leaves_of_Tree(T->lchild)+Leaves_of_Tree(T->rchild);
        // printf("num==%d\n",num );
        if(T->lchild==NULL&&T->rchild==NULL) return num+1;
        else return num;
    } 
